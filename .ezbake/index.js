...
const checkOrgLoading = () => {
  return new Promise((resolve, reject) => {
    ...
    const foo = () => {
      ...
      if (!orgsLoaded) {
        ui.updateBottomBar('\n  ' + chalk.yellow(loader[i++ % 4] + ' Still working to retrieve list of your authorized orgs...\n\n'));
        timeout = setTimeout(foo, 200);
      } else {
        ui.updateBottomBar('');
        ui.writeLog('\n  . ' + chalk.green('Org list retrieved successfully!\n'));
        resolve();
      }
    };
    foo();
...
module.exports = {
  ...
  ingredients: [{
      ...
      "message": "Have you authorized the Dev Hub and other orgs you intend to use in this project? (Yes/No)",
      "default": "No",
      validate: function (answer, answers) {
        ...
        _answers = answers || {};
        ui.writeLog('\n  . ' + chalk.yellow('Initializing process to retrieve list of your authorized orgs...\n'));
        fetchAllOrgs()
          .catch(err => {
            throw err;
          })
